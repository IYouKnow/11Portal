...
func New(cfg config.Config, dataStore *store.Store) *fiber.App {
	...
	notesHandler := handlers.NewNotesHandler(dataStore)
	...
	secured.Post("/network/scan", networkScanHandler.Scan)
	secured.Get("/notes", notesHandler.List)
	secured.Post("/notes", notesHandler.Create)
	secured.Patch("/notes/:id", notesHandler.Update)
	secured.Delete("/notes/:id", notesHandler.Delete)
